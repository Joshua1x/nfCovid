...
exec 3>&1 4>&2
#exec 1> ${project_path}/sumcovidseq/logs/prepare_submission_sh.log
#exec 2> ${project_path}/sumcovidseq/logs/prepare_submission_sh.log
...
do
   ...
   if [ ${num_actg} -ge 15000 ]
     then
      ...
      genbank_head=$(echo '>'"${sub_id}"' [keyword=purposeofsampling:baselinesurveillance][Country=USA][Host=Human][Isolate=SARS-CoV-2/Human/USA/'"${sub_id}"'/'"2023"'][Collection-Date='"${collection_date}"']' )
        echo $genbank_head
   ...
   # GISAID. GISAID has a lower limit of 25K actg nucleotides (non-ambiguous) for SARS-CoV-2 sequences
   if [ ${num_actg} -ge 25000 ]
    then
      gisaid_head=$(echo '>hCoV-19/USA/'"${sub_id}"'/'2023)
